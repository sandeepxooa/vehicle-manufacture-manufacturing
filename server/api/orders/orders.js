  ...
  request.get(
    {
      ...
      json: true,
      headers: {
        Authorization: restServerConfig.authToken
      }
    },
      ...
      request.get(
        ...
        (err, response, updates) => {
          if (Array.isArray(placedOrders) && Array.isArray(updates)) {
            placedOrders.forEach(order => {
              ...
              for (var i = updates.length - 1; i >= 0; i--) {
                ...
                if (typeof update.order === "string") {
                  updatingId = update.order.replace(
                    "resource:org.acme.vehicle_network.Order#",
                    ""
                  );
            ...
            res
              .status(500)
              .send("Response from rest server was not in format expected.");
          }
        }
      );
    }
